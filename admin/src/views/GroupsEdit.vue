<template>
    <v-container>
        <h1 class="d-flex">
            <span>Groupe :</span>
            <v-form class="pl-3" @submit="renameGroup(group.group_id)"><v-text-field v-model="group.group_name" :disabled="disabled == 1"></v-text-field></v-form>
        ...
        <v-table>
            ...
            <tbody>
                <tr
                    ...
                    :key="i"
                >
                    <td>{{ user }}</td><td><v-btn size="small" class="float-right" flat icon=mdi-minus @click="deleteUser(user.users_names)"></v-btn></td>
                </tr>
            </tbody>
        </v-table>
    </v-container>
